#!/usr/bin/env bash
# 手机端 OpenCL 本地推理 baseline
# 用法：./run_phone_opencl_baseline.sh [ngl] [prompt] [n]
#
...
BIN="${PHONE_LLAMA_CPP_DIR}/build-opencl/bin/llama-completion"

LOG_DIR="${SCRIPT_DIR}/../logs"
mkdir -p "${LOG_DIR}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${LOG_DIR}/phone_opencl_baseline_${TIMESTAMP}.log"
...
if [ ! -x "${BIN}" ]; then
  ...
  echo "Build with:" >&2
  echo "  cd ${PHONE_LLAMA_CPP_DIR} && cmake -B build-opencl -DGGML_OPENCL=ON -DGGML_OPENCL_USE_ADRENO_KERNELS=OFF && cmake --build build-opencl --target llama-completion -j" >&2
  exit 1
fi

exec "${BIN}" -m "${PHONE_MODEL_PATH}" -ngl "${NGL}" -p "${PROMPT}" -n "${N}" -no-cnv -v
